...
def on_info_button1_hover(event, nombre):
    ...
    # Función que se ejecuta cuando se hace clic en el botón de información del botón 2


def on_info_button5_hover(event):
    global info_label
    destroy_info_label()
    show_info_label("Selecciona VB-Cable Input (Normalmente el C)")


def on_info_button6_hover(event):
    global info_label
    destroy_info_label()
    show_info_label("Selecciona tu entrada de Audio")
...
def Distancia(slider, clicked_coordinates, root):
    ...
    if faces:
        ...
        factorVolumen = (((int(Dist + distmicrocara)) / 50)) * 100
